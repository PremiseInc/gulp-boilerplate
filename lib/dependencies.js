...
const searchConfig = {
	...
	'.scss': {
		...
		resolvers: [
			...
			filename => filename.replace( /([^\/]+)$/, '_$1.scss' ),
			// Try with SASS index filename
			filename => filename + '/_index.scss',
		],
	},
};
...
function resolveDependencyReference( input, resolvers ) {
	// Exact filename exists, use that
	if ( fs.existsSync( input ) ) {
		return input;
	}

	// Run through each resolver,
	// Return the first valid filename found
...
module.exports = function getDependencies( filename ) {
	...
	const stats = fs.fstatSync( filename );
